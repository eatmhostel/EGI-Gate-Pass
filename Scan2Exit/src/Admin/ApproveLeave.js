...
export default function ApproveLeave() {
    ...
    const fetchRequests = useCallback(async (isRefresh = false) => {
        ...
        try {
            const res = await fetch(`${EXPO_PUBLIC_API_URL}/gatepass/pending-home`);
    ...
    const handleApprove = (item) => {
        Alert.alert(
            "Approve Home Leave",
            `Approve home leave request for ${item.student?.fullName || item.student?.name || "this student"}?`,
            [
                { text: "Cancel", style: "cancel" },
                {
                    text: "Approve",
                    style: "default",
                    onPress: () => performAction(item._id, "approve"),
                },
            ]
        );
    };
    ...
    const handleReject = (item) => {
        Alert.alert(
            "Reject Home Leave",
            `Reject home leave request for ${item.student?.fullName || item.student?.name || "this student"}?`,
            [
                { text: "Cancel", style: "cancel" },
                {
                    text: "Reject",
                    style: "destructive",
                    onPress: () => performAction(item._id, "reject"),
                },
            ]
        );
    };
    ...
    const performAction = async (id, action) => {
        ...
        try {
            const endpoint =
                action === "approve"
                    ? `${EXPO_PUBLIC_API_URL}/gatepass/approve/${id}`
                    : `${EXPO_PUBLIC_API_URL}/gatepass/reject/${id}`;
                ...
                {!loading && requests.length === 0 && (
                    <View style={styles.emptyState}>
                        ...
                        <Text style={styles.emptySubtitle}>
                            No pending home leave requests at the moment.
                        </Text>
                    </View>
                )}
                                    ...
                                    <>
                                        ...
                                        <View style={styles.reasonRow}>
                                            ...
                                            <Text style={styles.reasonText}>
                                                {item.reason || item.purpose || "General"}
                                            </Text>
                                        </View>
                                    </>
                                )}
